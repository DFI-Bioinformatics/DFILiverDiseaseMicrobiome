...
# Figure 6 + Supp Figure 8: analysis on SBP vs. No SBP 
...
# subset to SBP cohort ----------------------------------------------------

meta <- metab %>% 
  filter(!is.na(sbp_diag))
...
taxgg <- t %>%
  left_join(meta %>% distinct(ID, bifido_percent = Bifidobacterium, sbp_diag)) %>%
...
pdf("./results/5.LD111.sbp.metaphlan.sbp.pdf",
    height = 6.25, width = 12.5)
...
# lefse analysis between sbp vs. no sbp --------------------------------------
...

{
  ...
  ggsave(paste0("./results/5.LD111.spb.",lubridate::today(),".lefse_analysis.pdf"),
...
metab.long %>%
  ...
  labs(x = "Bifido", y = "Metabolites Conc.\n(log transformed)",
       caption = subtext) +
  scale_y_continuous(trans = yingtools2::log_epsilon_trans(0.1))

ggsave("./results/5.LD111.SBP.metabolites.qunat.pdf", width = 12.5, height = 9.3)

# quant bile acid ratios --------------------------------------------------

ratio %>% 
  ggplot(aes(sbp_diag, value, fill = sbp_diag)) +
...
ggsave("./results/5.LD111.sbp_BAratios.boxplot.pdf", 
...
ggsave(plot = volcano, file.path("./results/5.LD111.volcano_metab_sbp.qual.pdf"),
